...
################################################################################
if {[::legacy::find -design design:adc] ne ""} {
  puts "** A design with the same name is already loaded. It will be removed. **"
  delete_obj design:adc
}
...
# Libraries
################################################################################
::legacy::set_attribute library {/home/cae3/Desktop/ADC/ASIC_ADC/Synthesis/lib/sky130_tt_1.8_25_nldm.lib} /


# Design
################################################################################
read_netlist -top adc genus_invs_des/genus.v
...
# slack
set _slk_ [::legacy::get_attribute slack design:adc]
if {[regexp {^-?[0-9.]+$} $_slk_]} {
  set _slk_ [format %.1f $_slk_]
}
if {$_slk_ != "295.2"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden slack: 295.2,  current slack: $_slk_"
}
unset _slk_
# multi-mode slack
# tns
set _tns_ [::legacy::get_attribute tns design:adc]
if {[regexp {^-?[0-9.]+$} $_tns_]} {
...
unset _tns_
# cell area
set _cell_area_ [::legacy::get_attribute cell_area design:adc]
...
unset _cell_area_
# net area
set _net_area_ [::legacy::get_attribute net_area design:adc]
